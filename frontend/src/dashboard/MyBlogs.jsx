...
const MyBlogs = () => {
  ...
  useEffect(() => {
    const fetchMyBlogs = async () => {
      try {
        const { data } = await axios.get("https://blog-app-s1n1.onrender.com/api/blogs/my-blog", {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`https://blog-app-s1n1.onrender.com/api/blogs/delete/${id}`, {
        withCredentials: true
      });
  ...
  return (
    <div className="container mx-auto my-12 p-4">
      <h1 className="font-semibold text-center m-2">My Blogs</h1>
